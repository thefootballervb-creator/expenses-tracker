                ...
                <RoutesWrapper isDarkMode={isDarkMode}>
                    <Routes>
                        ...
                        <Route element={<ProtectedRoute isAllowed={(() => {
                            const user = AuthService.getCurrentUser();
                            console.log('Admin route check - User:', user ? user.email : 'No user');
                            console.log('Admin route check - User roles:', user?.roles);
                            console.log('Admin route check - Roles type:', typeof user?.roles, Array.isArray(user?.roles));
                            
                            const hasAdminRole = user && user.roles && Array.isArray(user.roles) && user.roles.includes("ROLE_ADMIN");
                            if (!hasAdminRole && user) {
                                console.warn('Admin route access denied - User roles:', user.roles);
                                console.warn('Admin route access denied - Please log out and log back in to refresh your session with ROLE_ADMIN');
                            }
                            return hasAdminRole;
                        })()} />}>
