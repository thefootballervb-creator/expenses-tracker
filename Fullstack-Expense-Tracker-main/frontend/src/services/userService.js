...
const getMonthlySummary = async (email) => {
    ...
    if (!user || !user.token) {
        console.error('getMonthlySummary - No valid user token found!')
        throw new Error('User not authenticated')
    }
    
    const headers = AuthService.authHeader()
    console.log('getMonthlySummary - Headers:', headers)
    console.log('getMonthlySummary - Email:', email)
    console.log('getMonthlySummary - Token present:', !!headers.Authorization)
...
const downloadTransactionsPdf = async (email) => {
    const user = AuthService.getCurrentUser();
    if (!user || !user.token) {
        console.error('Download PDF - No user or token found!');
        throw new Error('Authentication required. Please log out and log back in.');
    }
    
    const headers = { Authorization: 'Bearer ' + user.token };
    console.log('Download PDF - Email:', email);
    console.log('Download PDF - Token present:', !!user.token);
    console.log('Download PDF - Token length:', user.token ? user.token.length : 0);
    
    return await axios.get(
        API_BASE_URL + '/report/exportTransactions/pdf',
        {
            headers: headers,
            params: { email },
            responseType: 'blob'
...
const downloadTransactionsExcel = async (email) => {
    const user = AuthService.getCurrentUser();
    if (!user || !user.token) {
        console.error('Download Excel - No user or token found!');
        throw new Error('Authentication required. Please log out and log back in.');
    }
    
    const headers = { Authorization: 'Bearer ' + user.token };
    console.log('Download Excel - Email:', email);
    console.log('Download Excel - Token present:', !!user.token);
    console.log('Download Excel - Token length:', user.token ? user.token.length : 0);
    
    return await axios.get(
        API_BASE_URL + '/report/exportTransactions/excel',
        {
            headers: headers,
            params: { email },
            responseType: 'blob'
